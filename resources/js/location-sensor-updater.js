 ...
 * Hybrid approach: Firebase listener for real-time + polling for reliability
 * Updates sensor cards on location detail page with real-time data
 */
...
const LocationSensorUpdater = {
  ...
  /**
   * Fetch sensor data from API (fallback method with deep comparison)
   */
  updateSensorData() {
    try {
      const timestamp = Date.now();
  ...
  destroy() {
    ...
    if (this.pollInterval) {
      clearInterval(this.pollInterval);
      console.log('[LocationSensorUpdater] Polling stopped');
    }
